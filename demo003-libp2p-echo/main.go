...
func main() {
	...
	listenPort := flag.Int("l", 0, "wait for incoming connections")
	targetAddr := flag.String("d", "", "target peer to dial")
	insecureFlag := flag.Bool("insecure", false, "use an unencrypted connection")
	seedFlag := flag.Int64("seed", 0, "set random seed for id generation")
	flag.Parse()
	...
	ha, err := makeBasicHost(*listenPort, *insecureFlag, *seedFlag)
	if err != nil {
		log.Fatal(err)
	}

	if *targetAddr == "" {
		startListener(ctx, ha, *listenPort, *insecureFlag)
		// Run until canceled.
		<-ctx.Done()
	} else {
		runSender(ctx, ha, *targetAddr)
	}
}
...
func runSender(ctx context.Context, ha host.Host, targetPeer string) {
	...
	ha.Peerstore().AddAddrs(info.ID, info.Addrs, 1000*1000)
