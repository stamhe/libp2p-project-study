...
func (p *NodeProtocol) onFirstP2PMsg(s network.Stream) {
	...
	msg := fmt.Sprintf("hello-start-hole-punch-%s", p.node.chatP2PAddr.ID.String())
	p.node.sendMessage(p.node.chatP2PAddr.ID, holepunch.Protocol, msg)
...
func (n *Node) ConnectRelay(relay string) error {
	log.Println("start ConnectRelay....", relay)
...
func (n *Node) ConnectNode(nodePeerID string, nodeAddr string) {
	...
	var address = nodeAddr + "/p2p/" + n.ServerAddrInfo.ID.String() + "/p2p-circuit/p2p/" + peerID.String()
	...
	log.Println("send hello to", nodePeerID)
	n.sendMessage(chatTargetAddrInfo.ID, FirstP2PMsg, "Hello-"+chatTargetAddrInfo.ID.String())
	//n.CmdRelay()
}

func (n *Node) CmdP2P() {
	log.Println("Cmdp2p start ....", n.chatP2PAddr.ID.String())
	...
	rw.WriteString("Hello, this is msg from " + n.ID().String())
	rw.Flush()
	go n.writeData(rw)
	go n.readData(rw)
}
...
func (n *Node) Login() {
	if n.sendMessage(n.ServerAddrInfo.ID, RelayProtocolRequest, "login") {
		log.Printf("login to relay, %s\n", n.ID())
	} else {
		log.Printf("connect to relay error, %s\n", n.ID())
	}
}
...
func (n *Node) sendMessage(id peer.ID, p protocol.ID, data string) bool {
	...
	if err != nil {
		log.Println("sendMessage NewStream err,", data, err)
		return false
	}
	defer s.Close()
...
func makeRandomNode(relay string) *Node {
	...
	h, err := libp2p.New(
		...
		libp2p.EnableAutoRelayWithStaticRelays([]peer.AddrInfo{*peerInfo}),
